...
export default function DownloadReport() {
  ...
  useEffect(() => {
    async function downloadPdf() {
      if (!rows.length || !reportRef.current || downloading || downloadedRef.current) return;
      downloadedRef.current = true;
      setDownloading(true);
      setTimeout(async () => {
        await downloadPagesAsPdf(reportRef.current.querySelectorAll('.pdf-report-page'), 'reports.pdf');
  ...
  function imageUrl(path) {
    if (!path) return '';
    return `${api.defaults.baseURL}/reports/image?path=${encodeURIComponent(path)}`;
  }
  ...
  for (let index = 0; index < rows.length; index += rowsPerPage) {
    reportPages.push(rows.slice(index, index + rowsPerPage));
  }

  return html`
    <div>
      <div className="pdf-download-status">${downloading ? 'Preparing PDF...' : rows.length ? 'PDF downloaded. You may close this tab.' : 'Loading records...'}</div>
      <div className="pdf-render-workspace">
      <div ref=${reportRef} className="pdf-report-pages">
        ${reportPages.map((pageRows, pageIndex) => html`
          <div key=${pageIndex} className="print-report-page pdf-a4-page pdf-report-page">
            ${pageIndex === 0 && html`
